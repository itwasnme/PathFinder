...
void static SendByte(uint8_t byte){
    ...
    uint8_t copy = byte; //Make a copy of data
    int i;
    for (i = 0; i<8; i++){ //Repeat for all 8 bits of data
        if((copy>>7) == 1){ //Shift 7 right to check if the MSB is 1
            LCD_SEND_1;  //If 1, store 1 on the MOSI line
        }
        else{
            LCD_SEND_0; //If 0, store 1 on the MOSI line
        }
        copy = copy<<1; //Update to check for following bits

        //triggering one cycle of the clock//
        LCD_TOGGLE_SCLK;
        LCD_TOGGLE_SCLK;
    }
}
...
void static LCDCommandWrite(uint8_t command){
    // write code here for LCD lab
    LCD_ENABLE_MODULE; // Enable LCD Module
    SET_DC_FOR_COMMAND; // Set DC bit for command
    SendByte(command); //Send command byte (parameter^)
    LCD_DISABLE_MODULE; //Disable LCD Module
}
...
void static LCDDataWrite(uint8_t data){
    // write code here for LCD lab
    LCD_ENABLE_MODULE; // Enable LCD Module
    SET_DC_FOR_DATA; // Set DC bit for data
    SendByte(data); //Send data byte (parameter^)
    LCD_DISABLE_MODULE; //Disable LCD Module
}
